        ...
        if Var.PRIVATE_GROUP_ID:
            ...
            if not event.is_private:
                ...
                try:
                    ...
                    await hydra.send_message(
                        Var.PRIVATE_GROUP_ID,
                        f"Please add {MYBOT} here for afk tags to work.",
                    )


@hydra.on(admin_cmd(pattern=r"afk ?(.*)"))
async def _(event):
    if event.fwd_from:
        return
    global USER_AFK
    ...
    if not USER_AFK:
        if event.reply_to_msg_id:
            ...
            media = await hydra.download_media(reply_message, "AFK_media")
            try:
...
@hydra.on(events.NewMessage(outgoing=True))
